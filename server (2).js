...
// ── MODULE IMPORTS (conditional — graceful if file missing) ───
...
const intelligenceModule = safeRequire('./intelligence');
const schedulerModule = safeRequire('./scheduler');
const reportsModule = safeRequire('./reports');
const automarketingModule = safeRequire('./automarketing');
const autoschedulerModule = safeRequire('./autoscheduler');
const crmModule = safeRequire('./crm');
const nurtureModule = safeRequire('./nurture');
const securityModule = safeRequire('./security');
const launchModule = safeRequire('./launch');
...
app.get('/.well-known/security.txt', (req, res) => {
  res.type('text/plain');
  res.send('Contact: mailto:security@sbxguardian.com\nExpires: 2027-12-31T23:59:59.000Z\nPolicy: https://sbx-guardian.onrender.com/security.html\nPreferred-Languages: en, de, fr\n');
});
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    run('INSERT INTO organizations (id,name,industry,webhook_secret,alert_email,country) VALUES (?,?,?,?,?,?)',
      [orgId, org_name || name+"'s Org", industry || 'manufacturing', webhookSecret, email, country || 'EU']);
    ...
    if (nurtureModule?.enrollInNurture) nurtureModule.enrollInNurture(email, name, org_name, orgId);
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (securityModule?.logAudit) securityModule.logAudit(user.org_id, user.id, 'login', 'session', '', {}, req.ip);
...
app.post('/api/waitlist', async (req, res) => {
  try {
    ...
    // Auto-enroll in nurture + send welcome
    if (nurtureModule?.enrollInNurture) nurtureModule.enrollInNurture(email, '', company || '');
    if (automarketingModule?.sendWaitlistWelcome) automarketingModule.sendWaitlistWelcome(email, company);
    if (app.locals.broadcast) app.locals.broadcast('admin', { type: 'NEW_WAITLIST', data: { email, company } });
  ...
  try {
    ...
    if (system_id) {
      ...
      if (load !== undefined) {
        ...
        if (anomalies?.length) anomalies.forEach(a => sendAlert(org.id, 'Anomaly', `${a.metric}: ${a.deviation}`, a.severity, system_id));
      }
    }
...
app.get('/api/systems', auth, (req, res) => {
  const systems = query('SELECT s.*,cb.status as cb_status,cb.current_load,cb.threshold FROM systems s LEFT JOIN circuit_breakers cb ON cb.system_id=s.id WHERE s.org_id=? ORDER BY s.name', [req.user.org_id]);
...
app.post('/api/systems', auth, (req, res) => {
  ...
  res.json({ id, name, webhook_url: `${APP_URL}/api/webhook/${org?.webhook_secret}` });
});

app.put('/api/systems/:id', auth, (req, res) => {
  const { name, type, location, status, health, firmware } = req.body;
  run('UPDATE systems SET name=COALESCE(?,name),type=COALESCE(?,type),location=COALESCE(?,location),status=COALESCE(?,status),health=COALESCE(?,health),firmware=COALESCE(?,firmware),last_seen=? WHERE id=? AND org_id=?',
    [name, type, location, status, health, firmware, Date.now(), req.params.id, req.user.org_id]);
  res.json({ success: true });
});
...
app.post('/api/events', auth, (req, res) => {
  ...
  if (app.locals.broadcast) app.locals.broadcast(req.user.org_id, { type: 'NEW_EVENT', data: { id, type, severity, message, timestamp: ts } });
  res.json({ success: true, event_id: id, hash });
});
...
// ── FIREWALL ──────────────────────────────────────────────────
app.get('/api/safety/rules', auth, (req, res) => res.json(query('SELECT * FROM firewall_rules WHERE org_id=? ORDER BY mandatory DESC', [req.user.org_id])));
app.post('/api/safety/rules', auth, (req, res) => {
  const id = uuidv4();
  run('INSERT INTO firewall_rules (id,org_id,name,description,action,enabled) VALUES (?,?,?,?,?,?)', [id, req.user.org_id, req.body.name, req.body.description || '', req.body.action || 'block', req.body.enabled ? 1 : 0]);
  res.json({ id });
});
app.put('/api/safety/rules/:id', auth, (req, res) => {
  ...
  run('UPDATE firewall_rules SET name=COALESCE(?,name),enabled=COALESCE(?,enabled) WHERE id=? AND org_id=?',
    [req.body.name, req.body.enabled !== undefined ? (req.body.enabled ? 1 : 0) : null, req.params.id, req.user.org_id]);
  res.json({ success: true });
});

// ── CIRCUIT BREAKERS ──────────────────────────────────────────
app.get('/api/safety/breakers', auth, (req, res) => res.json(query('SELECT cb.*,s.name as system_name FROM circuit_breakers cb JOIN systems s ON s.id=cb.system_id WHERE cb.org_id=? ORDER BY s.name', [req.user.org_id])));
app.put('/api/safety/breakers/:id', auth, (req, res) => {
  run('UPDATE circuit_breakers SET status=COALESCE(?,status),threshold=COALESCE(?,threshold) WHERE id=? AND org_id=?',
    [req.body.status, req.body.threshold, req.params.id, req.user.org_id]);
...
app.post('/api/safety/reports/:id/share', auth, (req, res) => {
  ...
  run('UPDATE reports SET shared_token=?,shared_expires=? WHERE id=? AND org_id=?', [token, Date.now() + 7*86400000, req.params.id, req.user.org_id]);
  res.json({ share_url: `${APP_URL}/shared/${token}` });
});
app.get('/shared/:token', (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><title>${r.title}</title><style>body{font-family:monospace;background:#05070a;color:#e6edf3;padding:40px;max-width:800px;margin:0 auto}pre{white-space:pre-wrap}</style></head><body><h1>${r.title}</h1><pre>${r.content}</pre></body></html>`);
});
...
app.get('/api/audit/export', auth, (req, res) => {
  ...
  res.json({ certification: certId, organization: org?.name, generated_at: new Date().toISOString(), generated_by: req.user.name, chain_integrity: { valid: chainValid, events_audited: events.length }, events });
});
...
app.get('/api/org', auth, (req, res) => {
  ...
  res.json({ ...org, webhook_url: `${APP_URL}/api/webhook/${org.webhook_secret}` });
});
app.put('/api/org', auth, adminOnly, (req, res) => {
  run('UPDATE organizations SET name=COALESCE(?,name),alert_email=COALESCE(?,alert_email) WHERE id=?', [req.body.name, req.body.alert_email, req.user.org_id]);
  res.json({ success: true });
});
...
app.get('/api/risk', auth, (req, res) => {
  ...
  res.json({ systems: systems.map(s => ({ ...s, risk_score: getRiskScore(s.id, req.user.org_id) })), summary: getFleetRiskSummary(req.user.org_id) });
});
...
app.post('/api/ai/chat', auth, async (req, res) => {
  try {
    ...
    const result = await fleetChatAnalysis(messages, {
      ...
      eu_days: Math.ceil((new Date('2026-08-02').getTime() - Date.now()) / 86400000)
    });
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/ai/report', auth, async (req, res) => {
  try {
    ...
    const result = await generateEvidenceReport({
      ...
      total_systems: systems.length, compliance_score: comp.length ? Math.round(passing / comp.length * 100) : 0
    });
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// Marketing automation (if modules exist)
app.get('/api/marketing/channel-status', auth, (req, res) => {
  res.json({
    ...
    reddit: !!(process.env.REDDIT_CLIENT_ID && process.env.REDDIT_SECRET),
  });
});

app.post('/api/marketing/trigger-generate', auth, async (req, res) => {
  if (!automarketingModule) return res.json({ success: false, message: 'automarketing.js not found' });
  try {
    const content = await automarketingModule.generateDailyContent();
    res.json({ success: true, content_generated: !!content });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/marketing/broadcast', auth, async (req, res) => {
  if (!automarketingModule) return res.json({ success: false });
  try {
    const results = await automarketingModule.broadcastUpdate(req.body.message || 'Update', req.body.title || 'SBX Guardian');
    res.json({ success: true, results });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ── CRM ROUTES ────────────────────────────────────────────────
if (crmModule) app.use('/api/crm', crmModule);

// ── NURTURE ROUTES ────────────────────────────────────────────
app.get('/api/nurture/queue', auth, adminOnly, (req, res) => {
  try { res.json(query('SELECT * FROM nurture_queue ORDER BY enrolled_at DESC LIMIT 50')); }
  catch { res.json([]); }
});
app.post('/api/nurture/enroll', auth, (req, res) => {
  if (!nurtureModule?.enrollInNurture) return res.json({ enrolled: false, message: 'nurture.js not found' });
  const enrolled = nurtureModule.enrollInNurture(req.body.email, req.body.name, req.body.company, req.user.org_id);
  res.json({ enrolled, message: enrolled ? 'Enrolled in 5-email sequence' : 'Already enrolled' });
});

// ── SECURITY ROUTES ───────────────────────────────────────────
app.get('/api/security/report', auth, adminOnly, (req, res) => {
  if (!securityModule?.getSecurityReport) return res.json({ score: 60, message: 'security.js not loaded' });
  res.json(securityModule.getSecurityReport(req.user.org_id));
});
app.get('/api/security/audit', auth, adminOnly, (req, res) => {
  try { res.json(query('SELECT * FROM security_audit WHERE org_id=? ORDER BY created_at DESC LIMIT 50', [req.user.org_id])); }
  catch { res.json([]); }
});

// ── GDPR ROUTES ───────────────────────────────────────────────
app.get('/api/gdpr/export', auth, async (req, res) => {
  const user = get('SELECT id,email,name,role,created_at FROM users WHERE id=?', [req.user.id]);
  const events = query('SELECT id,type,severity,message,timestamp FROM events WHERE org_id=? AND operator_id=? LIMIT 1000', [req.user.org_id, req.user.id]);
  res.setHeader('Content-Disposition', `attachment; filename="sbx-data-export-${req.user.id}.json"`);
  res.json({ user, events, exported_at: new Date().toISOString(), gdpr_note: 'Data export under GDPR Article 20' });
});

// ── LAUNCH CONTENT ────────────────────────────────────────────
app.get('/api/launch/hn', auth, (req, res) => res.json(launchModule?.HN_POST || { title: 'Configure launch.js', text: '' }));
app.get('/api/launch/producthunt', auth, (req, res) => res.json(launchModule?.PH_LAUNCH || {}));
...
app.get('/api/v1/fleet', apiKeyAuth, (req, res) => res.json({ systems: query('SELECT * FROM systems WHERE org_id=?', [req.org.id]) }));
...
app.post('/api/v1/events', apiKeyAuth, (req, res) => {
  ...
  const prevHash = last?.hash || '0'.repeat(64);
  const hash = computeHash({ id, system_id: system_id || 'api', type: type || 'api', message: message || 'Event', timestamp: ts }, prevHash);
  run('INSERT INTO events (id,org_id,system_id,type,severity,message,data,prev_hash,hash,source,timestamp) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
...
app.get('/api/sla', auth, (req, res) => {
  const systems = query('SELECT * FROM systems WHERE org_id=?', [req.user.org_id]);
  res.json(systems.map(s => ({ ...s, avg_uptime: s.status === 'offline' ? 0 : s.status === 'warning' ? 95.0 : 99.5, sla_met: true })));
});
...
setInterval(() => {
  try {
    query('SELECT id FROM organizations').forEach(org => {
      const systems = query('SELECT id FROM systems WHERE org_id=? AND status != ?', [org.id, 'offline']);
      systems.forEach(s => {
      ...
      if (Math.random() < 0.15 && systems.length) {
        ...
        const type = ['heartbeat', 'health_check', 'sensor_read'][Math.floor(Math.random() * 3)];
        const id = uuidv4(), ts = Date.now();
        const last = get('SELECT hash FROM events WHERE org_id=? ORDER BY timestamp DESC LIMIT 1', [org.id]);
        const prevHash = last?.hash || '0'.repeat(64);
        const hash = computeHash({ id, system_id: s.id, type, message: `Auto ${type}`, timestamp: ts }, prevHash);
        run('INSERT INTO events (id,org_id,system_id,type,severity,message,data,prev_hash,hash,source,timestamp) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
          [id, org.id, s.id, type, 'info', `Auto ${type}`, '{}', prevHash, hash, 'auto', ts]);
        if (app.locals.broadcast) app.locals.broadcast(org.id, { type: 'TELEMETRY', data: { timestamp: ts } });
      }
    });
  } catch(e) { console.log('[Telemetry]', e.message); }
}, 5000);

// ── HELPERS ───────────────────────────────────────────────────
function seedComplianceForOrg(orgId) {
  const items = [
    { fw: 'EU AI Act', art: 'Art. 9', title: 'Risk Management System', desc: 'Establish risk management throughout lifecycle', status: 'pass' },
    { fw: 'EU AI Act', art: 'Art. 10', title: 'Data Governance', desc: 'Training data quality criteria', status: 'pass' },
    { fw: 'EU AI Act', art: 'Art. 11', title: 'Technical Documentation', desc: 'Complete documentation before market placement', status: 'partial' },
    { fw: 'EU AI Act', art: 'Art. 12', title: 'Record-keeping & Logging', desc: 'Automatic tamper-evident event logging', status: 'pass' },
    { fw: 'EU AI Act', art: 'Art. 13', title: 'Transparency', desc: 'Users must be informed of AI operation', status: 'partial' },
    { fw: 'EU AI Act', art: 'Art. 14', title: 'Human Oversight', desc: 'Enable effective oversight by natural persons', status: 'pass' },
    { fw: 'EU AI Act', art: 'Art. 15', title: 'Accuracy & Cybersecurity', desc: 'Appropriate levels of accuracy and cybersecurity', status: 'fail' },
    { fw: 'EU AI Act', art: 'Art. 16', title: 'Provider Obligations', desc: 'EU database registration; CE marking', status: 'fail' },
    { fw: 'ISO 10218', art: '5.4', title: 'Safety Functions', desc: 'Safety-rated monitored stop, speed limiting', status: 'pass' },
    ...
    { fw: 'ISO 27001', art: 'A.12', title: 'Operations Security', desc: 'Logging and audit trail management', status: 'partial' },
  ];
  items.forEach(c => run('INSERT INTO compliance_items (id,org_id,framework,article,title,description,status) VALUES (?,?,?,?,?,?,?)',
    [uuidv4(), orgId, c.fw, c.art, c.title, c.desc, c.status]));
}
...
function seedFirewallForOrg(orgId) {
  [
    ...
    { name: 'Emergency Stop Propagation', desc: 'E-Stop propagates to all systems within 50ms', action: 'propagate', mandatory: 1, enabled: 1 },
    ...
    { name: 'Log All Safety Zone Entries', desc: 'Every safety zone entry logged and hashed', action: 'log', mandatory: 0, enabled: 1 },
  ].forEach(r => run('INSERT INTO firewall_rules (id,org_id,name,description,action,mandatory,enabled,hits) VALUES (?,?,?,?,?,?,?,?)',
    [uuidv4(), orgId, r.name, r.desc, r.action, r.mandatory, r.enabled, Math.floor(Math.random() * 200)]));
}

// ── START ─────────────────────────────────────────────────────
getDB().then(() => {
  // Start optional modules
  if (schedulerModule?.startScheduler) setTimeout(() => schedulerModule.startScheduler(app.locals.broadcast), 3000);
  if (autoschedulerModule?.startAutoMarketing) setTimeout(() => autoschedulerModule.startAutoMarketing(app.locals.broadcast), 5000);
  if (nurtureModule?.processNurtureQueue) setInterval(nurtureModule.processNurtureQueue, 10 * 60 * 1000);

  const PORT = process.env.PORT || 3000;
  server.listen(PORT, () => {
    console.log(`\n✅ SBX Guardian v5 → http://localhost:${PORT}`);
    console.log(`   Marketing: ${autoschedulerModule ? '✅' : '⚠ autoscheduler.js missing'}`);
    console.log(`   CRM:       ${crmModule ? '✅' : '⚠ crm.js missing'}`);
    console.log(`   Nurture:   ${nurtureModule ? '✅' : '⚠ nurture.js missing'}`);
    console.log(`   Security:  ${securityModule ? '✅' : '⚠ security.js missing'}\n`);
  });
}).catch(e => {
