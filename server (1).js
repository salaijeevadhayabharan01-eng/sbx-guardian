...
const { startAutoMarketing } = require('./autoscheduler');
const autoMarketing = require('./automarketing');
...
app.post('/api/marketing/trigger-generate', auth, async (req, res) => {
  try {
    const content = await autoMarketing.generateDailyContent();
    const seo = await autoMarketing.generateSEOArticle();
    res.json({ success: true, content_generated: !!content, seo_generated: !!seo });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/marketing/broadcast', auth, async (req, res) => {
  try {
    ...
    const results = await autoMarketing.broadcastUpdate(message||'Test broadcast', title||'SBX Guardian Update');
    res.json({ success: true, results });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/marketing/test-email', auth, async (req, res) => {
  try {
    const { to } = req.body;
    const sent = await autoMarketing.sendEmail(to, 'SBX Guardian — Test Email', '<h1 style="color:#00e87a">It works!</h1><p>Your email automation is configured correctly.</p>');
    res.json({ success: sent });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Start auto-marketing after 5 seconds
setTimeout(() => startAutoMarketing(app.locals.broadcast), 5000);

// ── NEW MODULE WIRING ─────────────────────────────────────────
const crmRouter = require('./crm');
const { processNurtureQueue, enrollInNurture } = require('./nurture');
const { getSecurityReport, calculateSecurityScore, logAudit, getAuditLog, generateAPIKey, validateAPIKey, securityHeadersMiddleware, auditMiddleware } = require('./security');
const { HN_POST, PH_LAUNCH, postToMastodon, MASTODON_POSTS } = require('./launch');

app.use(securityHeadersMiddleware);
app.use('/api/crm', crmRouter);

// Enroll waitlist signups in nurture automatically
const _origWaitlist = app._router?.stack?.find(r => r.route?.path === '/api/waitlist');
app.post('/api/waitlist-v2', async (req, res) => {
  try {
    const { email, company, role, systems_count, message } = req.body;
    if (!email) return res.status(400).json({ error: 'Email required' });
    if (get('SELECT id FROM waitlist WHERE email=?', [email])) return res.json({ success: true });
    run('INSERT INTO waitlist (id,email,company,role,systems_count,message) VALUES (?,?,?,?,?,?)',
      [uuidv4(), email, company||'', role||'', systems_count||'', message||'']);
    enrollInNurture(email, '', company||'');
    const { sendWaitlistWelcome } = require('./automarketing');
...
app.get('/api/security/report', auth, adminOnly, (req, res) => {
  res.json(getSecurityReport(req.user.org_id));
});

app.get('/api/security/audit', auth, adminOnly, (req, res) => {
  res.json(getAuditLog(req.user.org_id, parseInt(req.query.limit)||50));
});

app.post('/api/security/api-keys', auth, adminOnly, (req, res) => {
  const key = generateAPIKey(req.user.org_id, req.body.name, req.body.permissions);
  logAudit(req.user.org_id, req.user.id, 'api_key_created', 'api_key', '', { name: req.body.name }, req.ip);
  res.json({ key, warning: 'Save this key — it will never be shown again.' });
});

// GDPR ROUTES
app.get('/api/gdpr/export', auth, async (req, res) => {
  const { exportUserData } = require('./security');
  const data = await exportUserData(req.user.id, req.user.org_id);
...
app.delete('/api/gdpr/delete-account', auth, async (req, res) => {
  const { deleteUserData } = require('./security');
  await deleteUserData(req.user.id, req.user.org_id);
  res.json({ success: true, message: 'Account data anonymized per GDPR Article 17' });
});

// LAUNCH CONTENT ROUTES
app.get('/api/launch/hn', auth, (req, res) => res.json(HN_POST));
app.get('/api/launch/producthunt', auth, (req, res) => res.json(PH_LAUNCH));
app.post('/api/launch/mastodon', auth, async (req, res) => {
  const post = MASTODON_POSTS[Math.floor(Math.random() * MASTODON_POSTS.length)];
  const result = await postToMastodon(req.body.content || post);
  res.json({ success: !!result, url: result, post });
});

// NURTURE MANAGEMENT ROUTES
app.get('/api/nurture/queue', auth, adminOnly, (req, res) => {
  res.json(query('SELECT * FROM nurture_queue ORDER BY enrolled_at DESC LIMIT 50'));
});

app.post('/api/nurture/enroll', auth, (req, res) => {
  const { email, name, company } = req.body;
  const enrolled = enrollInNurture(email, name, company, req.user.org_id);
  res.json({ enrolled, message: enrolled ? 'Enrolled in 5-email sequence' : 'Already enrolled' });
});

// Run nurture queue every 10 minutes
setInterval(processNurtureQueue, 10 * 60 * 1000);
// Run once at startup
setTimeout(processNurtureQueue, 10000);
